...
async function gracefulShutdown(server, state, telemetry, options) {
    ...
    if (shutdownComplete) {
        console.log('[shutdown] Already completed, returning cached result');
        return shutdownComplete;
    }
    // If shutdown in progress, return existing promise
    if (shutdownInProgress) {
        console.log('[shutdown] Already in progress, waiting for completion');
        return shutdownInProgress;
    }
    // Start new shutdown sequence
    shutdownInProgress = performShutdown(server, state, telemetry, options);
    try {
 ...
 */
async function performShutdown(server, state, telemetry, options) {
 ...
 * @param telemetry - Telemetry system to shutdown
 */
function setupSignalHandlers(server, state, telemetry, onComplete, options) {
    const signals = ['SIGTERM', 'SIGINT'];
    const handle = async (signal, timeout) => {
        try {
            const result = await gracefulShutdown(server, state, telemetry, {
                signal,
