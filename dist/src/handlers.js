 ...
 */
function handleHome(ctx, runtimeState) {
...
<body>
  ...
  <ul>
    ...
    return (0, response_js_1.htmlResponse)(html, 200, {}, requestId);
}
/**
 * Handle GET /badge/:label/:value.svg - Generate SVG badge.
 */
function handleBadge(ctx) {
    const { label, value } = ctx.params;
    if (!label || !value) {
        const svg = generateErrorBadge('Invalid params');
        return (0, response_js_1.svgResponse)(svg, {}, ctx.id);
    }
