...
exports.createHighPerformancePipeline = createHighPerformancePipeline;
...
function createProductionPipeline() {
    const pipeline = new middleware_js_1.MiddlewarePipeline();
    // 1. Logging middleware (first in pipeline)
    // Logs all requests with timing and response details
    pipeline.use((0, logging_js_1.createLoggingMiddleware)({
        minLevel: logging_js_1.LogLevel.INFO,
    }));
    ...
    pipeline.use((0, compression_js_1.createCompressionMiddleware)({
        algorithm: compression_js_1.CompressionAlgorithm.GZIP, // Prefer gzip (widely supported)
 ...
 */
function createHighPerformancePipeline() {
    const pipeline = new middleware_js_1.MiddlewarePipeline();
    ...
    pipeline.use((0, compression_js_1.createCompressionMiddleware)({
        algorithm: compression_js_1.CompressionAlgorithm.GZIP,
 ...
 */
function createDevelopmentPipeline() {
    const pipeline = new middleware_js_1.MiddlewarePipeline();
    // Detailed logging with DEBUG level
    pipeline.use((0, logging_js_1.createLoggingMiddleware)({
        minLevel: logging_js_1.LogLevel.DEBUG,
    }));
