 ...
 */
function parseAcceptEncoding(acceptEncoding: string | undefined): string[] {
 ...
 */
export function createCompressionMiddleware(config: CompressionConfig = {}): MiddlewareHandler {
  ...
  return async (ctx, next) => {
    ...
    const acceptEncoding = forceCompression
      ? 'br, gzip'
      : (ctx.headers['accept-encoding'] as string);
    ...
    try {
      ...
      if (encoding === CompressionAlgorithm.GZIP) {
        compressedBody = await new Promise((resolve, reject) => {
          ...
          compressor.on('data', (chunk) => chunks.push(chunk as Buffer));
        ...
        compressedBody = await new Promise((resolve, reject) => {
          const compressor = createBrotliCompress({ params: { [0]: level } });
          const chunks: Buffer[] = [];
 ...
 */
export function createNoCompressionMiddleware(): MiddlewareHandler {
