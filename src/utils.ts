...
export function isValidHeaderValue(value: unknown): boolean {
  if (!value) return true; // Empty/null values are acceptable
 ...
 */
export function isValidHost(hostHeader: string, validHosts: string[]): boolean {
...
export function isValidContentType(contentType: unknown): boolean {
  if (!contentType) return true; // Empty is OK
 ...
 */
export function safeJsonParse<T>(json: string, fallback: T | undefined = undefined): T | undefined {
  try {
 ...
 */
export function mapOptional<T, R>(value: T | null | undefined, fn: (v: T) => R): R | undefined {
