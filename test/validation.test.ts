...
async function runTests(): Promise<void> {
  ...
  let response1 = await runtime1.handle(emptyEnvelope);
  assert.strictEqual(response1.statusCode, 200, 'Empty body should succeed');
  ...
  let response2 = await runtime2.handle(validHeadersEnvelope);
  assert.ok(response2.statusCode === 200, 'Valid headers should succeed');
